...
async def train_model(company_symbol: str):
    try:
        async with httpx.AsyncClient(timeout=200) as client:
            response = await client.get(f"{STOCK_FETCHER_SERVICE_URL}/stock-fetcher-service/ml-stock-data/?symbol={company_symbol}")
